...
# DISCLAIMER:
# This code file is forked from
# https://github.com/seongjunyun/Graph_Transformer_Networks


class GTN(nn.Module):

    def __init__(self, num_edge, num_channels, w_in, w_out, num_class,
        ...
        for i in range(num_layers):
            if i == 0:
                layers.append(GTLayer(num_edge, num_channels, num_nodes,
                                      first=True))
            else:
                layers.append(GTLayer(num_edge, num_channels, num_nodes,
                                      first=False))
    ...
    def norm(self, edge_index, num_nodes, edge_weight, improved=False,
             dtype=None):
        with torch.no_grad():
    ...
    def forward(self, A, X, target_x, target):
        ...
        for i in range(self.num_channels):
            if i == 0:
                ...
                X_ = self.gcn(X, edge_index=edge_index.detach(),
                ...
                X_ = torch.cat((X_,
                                F.relu(self.gcn(
                                        X, edge_index=edge_index.detach(),
                                        edge_weight=edge_weight))),
                               dim=1)
        X_ = self.linear1(X_)
        X_ = F.relu(X_)
            ...
            for j, (edge_index, edge_value) in enumerate(A):
                if j == 0:
                    ...
                    total_edge_index = torch.cat(
                            (total_edge_index, edge_index), dim=1)
                    total_edge_value = torch.cat(
                            (total_edge_value, edge_value*filter[i][j]))
            index, value = torch_sparse.coalesce(
                    total_edge_index.detach(), total_edge_value,
