        ...
        with tf.compat.v1.name_scope(self.name):
            ...
            if self.logging:
                for output in outputs:
                    tf.compat.v1.summary.histogram(self.name + '/outputs',
                                                   output)
            return outputs

    def _log_vars(self):
        for var in self.vars:
            tf.compat.v1.summary.histogram(self.name + '/vars/' + var,
                                           self.vars[var])
...
class GraphSampleConvolution(Layer):
    """Graph convolution layer."""
    def __init__(self, input_dim, output_dim, placeholders, support,
                 name='con', sparse_support=True, sparse_inputs=False,
                 act=tf.nn.relu, bias=False, dropout=False, featureless=False,
                 **kwargs):
...
class GraphSampleConvolutionReg(Layer):
    """Graph convolution layer."""
    def __init__(self, input_dim, output_dim, placeholders, support,
                 name='con_reg', sparse_support=True, sparse_inputs=False,
                 act=tf.nn.relu, bias=False, dropout=False, featureless=False,
                 **kwargs):
    ...
    def _call(self, inputs):
        ...
        n_v = tf.cast(tf.shape(input=self.support)[0], tf.float32)
        n_u = tf.cast(tf.shape(input=self.support)[1], tf.float32)

        mean_output = tf.reshape(tf.reduce_mean(input_tensor=output, axis=0),
                                 (1, -1))
        if self.sparse_support:
            mean_support = 1.0/tf.cast(n_v, tf.float32)*tf.sparse.reduce_sum(
                    self.support, axis=0)
        else:
            mean_support = 1.0/tf.cast(n_v, tf.float32)*tf.reduce_sum(
                    input_tensor=self.support, axis=0)
        diff = tf.reshape(mean_support, (-1, 1))*pre_sup - mean_output
        reg = 1.0 / tf.cast(n_u * self.output_dim, tf.float32) * tf.reduce_sum(
                input_tensor=diff * diff)
...
class GraphSampleConvolutionSkip(Layer):
    """Graph convolution layer."""
    def __init__(self, input_dim, output_dim, placeholders, support, hidden1,
                 name='con_reg', sparse_support=True, sparse_inputs=False,
                 act=tf.nn.relu, bias=False, dropout=False, featureless=False,
                 **kwargs):
        ...
        with tf.compat.v1.variable_scope('con_vars', reuse=True):
            w_12 = tf.compat.v1.get_variable('weights', [input_dim, hidden1])
        with tf.compat.v1.variable_scope('con_reg_vars', reuse=True):
            w_23 = tf.compat.v1.get_variable('weights',
                                             [hidden1, output_dim])
