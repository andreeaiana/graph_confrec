...
class MLP(Model):
    def __init__(self, placeholders, input_dim, learning_rate, weight_decay,
                 hidden1, **kwargs):
...
class GCNAdapt(Model):
    def __init__(self, placeholders, input_dim, learning_rate, weight_decay,
                 hidden1, var, sample_train=False, **kwargs):
        ...
        with tf.compat.v1.variable_scope(self.name, reuse=True):
            self.sample_params = tf.compat.v1.get_variable('sample_weights',
                                                           [input_dim, 2])

        self.support_21 = self._attention(self.supports[0], self.features[0],
    ...
    def _loss(self):
        ...
        self.loss += self.var*self.reg_loss
    ...
    def _attention(self, support, x_u, x_v, prob):
        ...
        n_v = tf.shape(input=support)[0]
        n_u = tf.shape(input=support)[1]
        ...
        attention = 1/tf.cast(n_u, tf.float32) * (
                tf.nn.relu(h_v + tf.reshape(h_u, (1, n_u))) + 1)  # v*u
        support = support*(attention/tf.reshape(prob, (1, n_u)))

        return support

    def _build(self):
        self.layers.append(GraphSampleConvolution(
...
class GCNAdaptMix(Model):
    ...
    def _attention(self, support, x_u, x_v, prob):
        sample_params = self.sample_params
        n_v = tf.shape(input=x_v)[0]
        n_u = tf.shape(input=x_u)[0]
        h_v = tf.matmul(x_v, tf.expand_dims(sample_params[:, 0], -1))  # v*1
        h_u = tf.matmul(x_u, tf.expand_dims(sample_params[:, 1], -1))  # u*1
        attention = 1/tf.cast(n_u, tf.float32) * (
                tf.nn.relu(h_v + tf.reshape(h_u, (1, n_u))) + 1)  # v*u
    ...
    def _build(self):
        ...
        self.layers.append(GraphSampleConvolutionReg(
                ...
                output_dim=self.output_dim,
                placeholders=self.placeholders,
                support=self._attention(
                        self.supports[0], self.features[0],
                        self.features[1], self.probs[0]),
                act=lambda x: x,
                dropout=True,
                logging=self.logging))
