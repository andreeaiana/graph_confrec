...
class Sampler(object):
    ...
    def __init__(self, placeholders, **kwargs):
        ...
        with tf.compat.v1.variable_scope(self.scope, reuse=True):
            self.w_s = tf.compat.v1.get_variable('sample_weights',
                                                 [self.input_dim, 2])

        self.adj = tf.Variable(placeholders['adj'], trainable=False,
                               name="adj")
        self.adj_val = tf.Variable(placeholders['adj_val'], trainable=False,
                                   name="adj_val")
        self.x = tf.Variable(placeholders['features'], trainable=False,
                             name="x")

        self.num_data = tf.shape(input=self.adj)[0]
...
class SamplerAdapt(Sampler):
    ...
    def sampling(self, v):
        ...
        for i in range(self.num_layers - 1):
            ...
            u_sampled, support, p_u = self.one_layer_sampling(
                    adj=this_adj, adj_val=this_adj_val, x_v=this_x_v,
    ...
    def one_layer_sampling(self, adj, adj_val, x_v, output_size):
        ...
        h_v = tf.reduce_sum(input_tensor=tf.matmul(
                x_v, tf.expand_dims(self.w_s[:, 1], -1)))
        h_u = tf.matmul(x_u, tf.expand_dims(self.w_s[:, 0], -1))
        attention = tf.reshape(
                1 / tf.cast(output_size, tf.float32) * (
                        tf.nn.relu(h_v + h_u) + 1), [-1])
        g_u = tf.reshape(tf.nn.relu(h_u) + 1, [-1])

        p1 = tf.cast(tf.sqrt(
                tf.sparse.reduce_sum(tf.square(support), axis=0)),
                dtype=tf.float32) * attention * g_u
        p1 = p1 / tf.reduce_sum(input_tensor=p1)

        samples = tf.cast(tf.random.categorical(
                logits=[tf.math.log(p1)], num_samples=output_size), tf.int64)
