...
class BipartiteEdgePredLayer(Layer):
    ...
    def neg_cost(self, inputs1, neg_samples, hard_neg_samples=None):
        """ For each input in batch, compute the sum of its affinity to
            negative samples.

        Returns:
            Tensor of shape [batch_size x num_neg_samples]. For each node,
                a list of affinities to negative samples is computed.
        """
        if self.bilinear_weights:
    ...
    def loss(self, inputs1, inputs2, neg_samples):
        ...
        Args:
            neg_samples: tensor of shape [num_neg_samples x input_dim2].
    ...
    def _xent_loss(self, inputs1, inputs2, neg_samples, hard_neg_samples=None):
        ...
        loss = tf.reduce_sum(input_tensor=true_xent) + self.neg_sample_weights * tf.reduce_sum(
                input_tensor=negative_xent)
        loss_node = true_xent + self.neg_sample_weights*tf.reduce_sum(
                input_tensor=negative_xent, axis=1)
    ...
    def _skipgram_loss(self, inputs1, inputs2, neg_samples,
        ...
        aff = self.affinity(inputs1, inputs2)
        neg_aff = self.neg_cost(inputs1, neg_samples, hard_neg_samples)
        neg_cost = tf.math.log(tf.reduce_sum(input_tensor=tf.exp(neg_aff),
                                             axis=1))
        loss = tf.reduce_sum(input_tensor=aff - neg_cost)
        loss_node = aff - neg_cost
        return loss, loss_node
    ...
    def _hinge_loss(self, inputs1, inputs2, neg_samples,
        ...
        neg_aff = self.neg_cost(inputs1, neg_samples, hard_neg_samples)
        diff = tf.nn.relu(
                tf.subtract(neg_aff, tf.expand_dims(aff, 1) - self.margin),
                name='diff')
