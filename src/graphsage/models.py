...
class Node2VecModel(GeneralizedModel):
    def __init__(self, placeholders, dict_size, degrees, name=None,
        ...
        Args:
            ...
            degrees: numpy array of node degrees, ordered as in the data's
                    id_map
    ...
    def _build(self):
        ...
        self.outputs1 = tf.nn.embedding_lookup(
                params=self.target_embeds, ids=self.inputs1)
        self.outputs2 = tf.nn.embedding_lookup(
                params=self.context_embeds, ids=self.inputs2)
        self.outputs2_bias = tf.nn.embedding_lookup(
                params=self.context_bias, ids=self.inputs2)
        self.neg_outputs = tf.nn.embedding_lookup(
                params=self.context_embeds, ids=self.neg_samples)
        self.neg_outputs_bias = tf.nn.embedding_lookup(
                params=self.context_bias, ids=self.neg_samples)

        self.link_pred_layer = BipartiteEdgePredLayer(
                self.hidden_dim, self.hidden_dim, self.placeholders,
                bilinear_weights=False)
    ...
    def _loss(self):
        aff = tf.reduce_sum(
                input_tensor=tf.multiply(self.outputs1, self.outputs2),
                axis=1) + self.outputs2_bias
        neg_aff = tf.matmul(
                self.outputs1, tf.transpose(a=self.neg_outputs)
                ) + self.neg_outputs_bias
        true_xent = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.ones_like(aff), logits=aff)
        negative_xent = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.zeros_like(neg_aff), logits=neg_aff)
        loss = tf.reduce_sum(input_tensor=true_xent) + tf.reduce_sum(
