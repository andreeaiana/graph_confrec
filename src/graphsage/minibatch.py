...
class EdgeMinibatchIterator(object):
    ...
    context_pairs -- if not none, then a list of co-occuring node pairs
                    (from random walks)
    ...
    def _remove_isolated(self, edge_list):
        ...
        for n1, n2 in edge_list:
            if n1 not in self.G.node or n2 not in self.G.node:
                missing += 1
                continue
            if (self.deg[self.id2idx[n1]] == 0
                or self.deg[self.id2idx[n2]] == 0) \
    ...
    def construct_adj(self):
        ...
        for nodeid in self.G.nodes():
            ...
            if len(neighbors) > self.max_degree:
                neighbors = np.random.choice(neighbors, self.max_degree,
                                             replace=False)
...
class NodeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx, placeholders, label_map, num_classes,
        ...
        self.train_nodes = [n for n in self.train_nodes if
                            self.deg[id2idx[n]] > 0]
